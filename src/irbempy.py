...
def _get_Lstar(ticks, loci, alpha, extMag='T01STORM', options=[1,0,0,0,0], omnivals=None): 
    ...
    # JGREEN This doesn't work because alpha is always a list
    # if isinstance(alpha, numbers.Number): # no drift shell splitting
    # so I changed it to check nalpha
    #if isinstance(alpha, numbers.Number): # no drift shell splitting
    #    raise Exception
    if nalpha==1:
    ...
    results = {}
    # PWHELAN - changed if to match JGREEN set of nalpha above
    #if nalpha == 0:
    if nalpha <= 1:
...
def get_Lstar(ticks, loci, alpha=90, extMag='T01STORM', options=[1,0,0,0,0], omnivals=None):
    ...
    def reassemble(result):
        ...
        #JGREEN: I added Blocal so this would work without shelsplitting
        # wether you need hstack or vstack depends on shelsplitting or no
        # I had to chane these all to hstack which probably won't work with
        # shell splitting
    ...
    if ncpus > 1 and ncalc >= ncpus*2:
        ...
        for block in range(nblocks):
            ...
            if block != nblocks-1: #not last block
                ...
                # 07/2021 JGREEN
                # block*blocklen gets you to startind
                # I'm having trouble when ncalc is 40 and ncpus is 16
                # blocklen is 2 even thouth the ncalc/ncpus is 2.5
                # So adding only 2* blocklen doesn't cut it here
                #endind = block*blocklen + 2*blocklen #going past the end of an array in a slice is fine
                endind = ncalc # Setting endind to ncalc ensures that this works
        ...
        inputs = [[tt[x], cc[x], alpha, extMag, options, ov[x]] for x in range(0,len(tt))]
