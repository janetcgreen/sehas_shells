...
def test_request_example2(client):
    ...
    test_data.close()
def test_request_early_times(client):
    # TEST: Check what happens when times are requested that are before any HAPI times
    # The code to create the test output file is called make_shells_testoutput.py
    # and it creates a files called shells_neural20220101.nc which has the
    # expected output for 20220101 and n15
    print('Testing response when times are before input data times')
    ...
    # Then create a list of times from the test file
    # This makes the times before the available inputs
    times1 = [dt.datetime.utcfromtimestamp(x / 1000)-dt.timedelta(hours=1) for x in test_data['time']]
    times2 = [x.strftime("%Y-%m-%dT%H:%M:%S.%fZ") for x in times1]
...
#      #NOTE: I temporarily commented this out becuase there is no data for
#       the sqlite dbase times at the moment
#     # TEST: Test that the HAPI request returns the right poes data
#     sdate = dt.datetime(2023,9,15,0,0,0)
...
def test_A_get_HAPI_tdiff(client):
    # TEST: Test that input data is flagged when it is before the returned
    # POES data

    # To test, give it a time we know exists
    sdate = dt.datetime(2023,9,15,0,0,0)
    times = [(sdate)+dt.timedelta(minutes=x*5) for x in range(0,256)]
    times2 = [x.strftime("%Y-%m-%dT%H:%M:%S.%fZ") for x in times]
    ...
    # This returns the hapi data inputs from sdate for a day
    hdata = pi.read_hapi_inputs(times2,server,dataset)

    # And then try to map that data to times that start 10 hours before
    times3=[(sdate-dt.timedelta(hours=10))+dt.timedelta(minutes=x*5) for x in range(0,256)]
    times4=[x.strftime("%Y-%m-%dT%H:%M:%S.%fZ") for x in times3]

    # The values before should be flagged -99
    hmap_data = pi.reorg_hapi(times4, hdata)

    # Check that the returned data is flagged
    inds = [x for x in range(0,len(times3)) if (times3[x]-sdate).total_seconds()<0 ]

    flagsum = 0
    for x in inds:
        for key in hmap_data.keys():
            if key !='time':
                flagsum =flagsum+np.sum(hmap_data[key][x])

    assert flagsum<0

def test_A_get_HAPI_tdiff_after(client):
    # TEST: Test that input data is flagged when it is more than 3 hours
    # after the last POES data

    # To test, give it a time we know exists in HAPI
    # Create a list of times for sep 25th to get from the HAPI dbase
    sdate = dt.datetime(2023,9,25,0,0,0)
    times = [(sdate)+dt.timedelta(minutes=x*5) for x in range(0,256)]
    times2 = [x.strftime("%Y-%m-%dT%H:%M:%S.%fZ") for x in times]

    # Load the info about the HAPI server and stuff
    load_dotenv(".env", verbose=True)
    server = os.environ.get('HAPI_SERVER')
    dataset = os.environ.get('HAPI_DATASET')
    ...
    # And then try to map that data to times that start 10 hours after
    times3=[(sdate+dt.timedelta(hours=10))+dt.timedelta(minutes=x*5) for x in range(0,256)]
    times4=[x.strftime("%Y-%m-%dT%H:%M:%S.%fZ") for x in times3]
    ...
    # Check that the returned data is flagged after
    inds = [x for x in range(0,len(times3)) if (times[-1]-times3[x]).total_seconds()<0 ]
...
def test_A_get_HAPI_tdiff_gap(client):
    # TEST: Test that input data is flagged when it is more than 3 hours
    # after the last POES data point when there is a gap
    ...
    # Now create a new hdata with a 5 hour gap
    newhdata = {}
    hinds = range(0,len(hdata['time'][:]))
    ninds = np.array(list(hinds[0:5])+list(hinds[25::]))
    for key in hdata.keys():
        newhdata[key] = hdata[key][ninds]

    # And then try to map that data to times that start 10 hours after
    times3=[sdate+dt.timedelta(minutes=x*5) for x in range(0,256)]
    times4=[x.strftime("%Y-%m-%dT%H:%M:%S.%fZ") for x in times3]

    # This should be the flagged values
    inds = [x for x in range(0,len(times3)) if (
            (times3[x]-dt.datetime.strptime(hdata['time'][4],'%Y-%m-%dT%H:%M:%SZ')).total_seconds()>3*3600)
            & (times3[x]<dt.datetime.strptime(hdata['time'][25],'%Y-%m-%dT%H:%M:%SZ'))]
    # The values before should be flagged -99
    hmap_data = pi.reorg_hapi(times4, newhdata)
