...
class IOList(MethodView):
    ...
    def post(self, io_data):
        ...
        if current_app.config["TESTING"]:
            # For testing we make up a set of L values and Bm values that
            # work for a fixed output testing file
...
class IOList(MethodView):
    ...
    def post(self, io_data):
        ...
        Required inputs:<br>
        - time: list of dates and times with format 'YYYY-MM-DDTHH:MM:SS.fffuuuZ'
        - Ls: list of L shells for returned electron flux (ex [5,6]
        - Bmirrors: Fixed list of mirror point magnetic fields (nT) for each L shell.
        (ex [100,150]).
        - energies: 1-D list of energies (keV) for the returned electron flux<br>
        <br>
        Outputs: (dictionary of arrays)
